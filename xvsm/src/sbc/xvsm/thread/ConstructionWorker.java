...
public class ConstructionWorker implements Runnable {
			...
			while (running) {
				...
				if (currentJob == null) {
					...
					while (currentJob.getQuantity() > 0) {
						...
						if (computerBuildt) {
							currentJob
									.setQuantity(currentJob.getQuantity() - 1);
						} else {
					...
					if (currentJob.getQuantity() <= 0) {
						capi.write(notficationContainer, new Entry(currentJob));
					}
				}
			}
		} catch (Exception e) {
	...
	private void initXvsm() throws Exception {
		...
		try {
			core = DefaultMzsCore.newInstance(SbcConstants.MAINPORT
					+ SbcConstants.CONSTRUCTIONPORTOFFSET);
			capi = new Capi(core);

			testContainer = capi.createContainer(SbcConstants.TESTERCONTAINER,
			...
			capi = new Capi(core);
			this.testContainer = capi
					.lookupContainer(
							SbcConstants.TESTERCONTAINER,
							new URI(
									"xvsm://localhost:+"
											+ (SbcConstants.MAINPORT + SbcConstants.CONSTRUCTIONPORTOFFSET)),
							MzsConstants.RequestTimeout.INFINITE, null);
		}
		System.out.println("xvsm://localhost:"
				+ (SbcConstants.MAINPORT + SbcConstants.LOGGERPORTOFFSET));
		this.notficationContainer = capi
				.lookupContainer(
						SbcConstants.NOTIFICATIONCONTAINER,
						new URI(
								"xvsm://localhost:"
										+ (SbcConstants.MAINPORT + SbcConstants.LOGGERPORTOFFSET)),
						MzsConstants.RequestTimeout.INFINITE, null);
		this.productionContainer = capi
				.lookupContainer(
						SbcConstants.PRODUCERCONTAINER,
						new URI(
								"xvsm://localhost:"
										+ (SbcConstants.MAINPORT + SbcConstants.PRODUCERPORTOFFSET)),
						MzsConstants.RequestTimeout.INFINITE, null);
		this.jobContainer = capi
				.lookupContainer(
						SbcConstants.JOBSCONTAINER,
						new URI(
								"xvsm://localhost:"
										+ (SbcConstants.MAINPORT + SbcConstants.PRODUCERPORTOFFSET)),
						MzsConstants.RequestTimeout.INFINITE, null);
		...
		capi.write(
				...
				new Entry(
						"ConstructionWorker: Setup complete, port: "
								+ (SbcConstants.MAINPORT + SbcConstants.CONSTRUCTIONPORTOFFSET)));
	}
		...
		try{
			...
			for(Job job: entryList){
				...
				if(canFulfillJob(job)){
					Property uuid = Property.forName("*", "uuid");
					Query query = new Query().filter(uuid.equalTo(job.getUuid()));
					
					ArrayList<Job> jobsTaken = capi.take(this.jobContainer, QueryCoordinator.newSelector(query), MzsConstants.RequestTimeout.TRY_ONCE, null);
					return job;
				}
			}
	...
	private void suspendJob(Job job) throws MzsCoreException,
			URISyntaxException {
	...
	private boolean canFulfillJob(Job job) throws MzsCoreException,
			URISyntaxException {
		TransactionReference tx = capi
				.createTransaction(
						MzsConstants.TransactionTimeout.INFINITE,
						new URI(
								"xvsm://localhost:"
										+ (SbcConstants.MAINPORT + SbcConstants.PRODUCERPORTOFFSET)));
		...
		cpuSelectors.add(LindaCoordinator.newSelector(new CpuComponent(null,
				null, null, config.getCpuType())));
		cpuSelectors.add(FifoCoordinator.newSelector());

		List<Selector> mainboardSelectors = new ArrayList<Selector>();
		mainboardSelectors.add(LindaCoordinator
				.newSelector(new MainboardComponent(null, null, null)));
		mainboardSelectors.add(FifoCoordinator.newSelector());

		List<Selector> ramSelectors = new ArrayList<Selector>();
		ramSelectors.add(LindaCoordinator.newSelector(new RamComponent(null,
				null, null)));
		ramSelectors
				.add(FifoCoordinator.newSelector(config.getRamModuleCount()));

		List<Selector> gpuSelectors = new ArrayList<Selector>();
		gpuSelectors.add(LindaCoordinator.newSelector(new GpuComponent(null,
				null, null)));
		gpuSelectors.add(FifoCoordinator.newSelector());

		try {
			ArrayList<CpuComponent> cpuResultEntries = capi.read(
					productionContainer, cpuSelectors,
					MzsConstants.RequestTimeout.TRY_ONCE, tx);
			ArrayList<MainboardComponent> mainboardResultEntries = capi.read(
					productionContainer, mainboardSelectors,
					MzsConstants.RequestTimeout.TRY_ONCE, tx);
			ArrayList<RamComponent> ramResultEntries = capi.read(
					productionContainer, ramSelectors,
					MzsConstants.RequestTimeout.TRY_ONCE, tx);
			ArrayList<GpuComponent> gpuResultEntries = null;

			if (config.isGraphicsCard()) {
				gpuResultEntries = capi.read(productionContainer, gpuSelectors,
						MzsConstants.RequestTimeout.TRY_ONCE, tx);
			}
	...
	private void buildDefault() throws MzsCoreException {
		...
		ramSelectorsGetTwoRams.add(LindaCoordinator.newSelector(
				new RamComponent(null, null, null), 2));
		...
		try {
			...
			ArrayList<CpuComponent> cpuResultEntries = capi.take(
					productionContainer, cpuSelectors,
					SbcConstants.CONSTRUCTION_TAKE_TIMEOUT, tx);
			ArrayList<MainboardComponent> mainboardResultEntries = capi.take(
					productionContainer, mainboardSelectors,
					SbcConstants.CONSTRUCTION_TAKE_TIMEOUT, tx);
			ArrayList<RamComponent> ramResultEntries = capi.take(
					productionContainer, ramSelectors,
					SbcConstants.CONSTRUCTION_TAKE_TIMEOUT, tx);
...
//				System.out.println("Got not all parts, rolling back transaction");
		...
		tx = capi.createTransaction(MzsConstants.TransactionTimeout.INFINITE,
				null);
		try {
			// Try to obtain optional part: GPU
			ArrayList<GpuComponent> gpuResultEntries = capi.take(
					productionContainer, gpuSelectors,
					SbcConstants.CONSTRUCTION_TAKE_TIMEOUT, null);
		...
		try {
			...
			ArrayList<RamComponent> ramResultEntries = capi.take(
					productionContainer, ramSelectorsGetTwoRams,
					SbcConstants.CONSTRUCTION_TAKE_TIMEOUT, null);
			ramComponents.add(ramResultEntries.get(0));
		...
		Computer computer = new Computer(cpuComponent, mainboardComponent,
				gpuComponent, ramComponents);
		Entry e = new Entry(computer);
		capi.write(testContainer, e);
		capi.write(notficationContainer, new Entry(
				"New computer constructed, no job"));
		System.out.println("New computer constructed, no job");
	}

	private boolean buildForJob(Job job) throws MzsCoreException,
			URISyntaxException {
		TransactionReference tx = capi
				.createTransaction(
						MzsConstants.TransactionTimeout.INFINITE,
		...
		try {
			...
			capi.commitTransaction(tx);
			Computer computer = new Computer(cpuComponent, mainboardComponent,
					gpuComponent, ramResultEntries);
			Entry e = new Entry(computer);
			capi.write(testContainer, e);
			capi.write(notficationContainer, new Entry(
					"New computer constructed, job uuid: " + job.getUuid()));
			System.out.println("New computer constructed, job uuid: "
					+ job.getUuid());
			return true;
		} catch (Exception e) {
			capi.rollbackTransaction(tx);
